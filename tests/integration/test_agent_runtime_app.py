...
def agent_app(monkeypatch: pytest.MonkeyPatch) -> AgentEngineApp:
    ...
    # Mock Cloud Logging so set_up() and register_feedback() don't hit GCP
    mock_log_client = unittest.mock.MagicMock()
    monkeypatch.setattr("google.cloud.logging.Client", lambda: mock_log_client)
...
async def test_agent_stream_query(agent_app: AgentEngineApp) -> None:
    ...
    # The workflow may emit a RequestInput interrupt (function_call part) when
    # no crop keyword is detected, or text content once advice is generated.
    # Accept either as a valid response.
    has_content = False
    for event in events:
        validated_event = Event.model_validate(event)
        content = validated_event.content
        if content is not None and content.parts and any(
            part.text or part.function_call is not None for part in content.parts
        ):
            has_content = True
            break

    assert has_content, "Expected at least one event with text or function_call content"
